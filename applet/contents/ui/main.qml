...
PlasmoidItem {
    ...
    function changeMicrophoneVolume(deltaPercent) {
        if (!Server.defaultSource) {
            return;
        }
        const newPercent = changeVolumeByPercent(Server.defaultSource, deltaPercent);
    ...
    function muteMicrophone() {
        ...
        var toMute = !Server.defaultSource.muted;
        Server.defaultSource.muted = toMute;
        osd.showMicMute(toMute? 0 : volumePercent(Server.defaultSource.volume));
    }
