...
class AppletTest(unittest.TestCase):
    ...
    pipewire_pulse: subprocess.Popen | None = None
    ...
    def setUpClass(cls) -> None:
        ...
        """
        cls.options.set_capability("app", f"plasmawindowed -p org.kde.plasma.nano {WIDGET_ID}")
        ...
        cls.options.set_capability("timeouts", {'implicit': 30000})
        cls.driver = webdriver.Remote(command_executor='http://127.0.0.1:4723', extensions=[SetValueCommand], options=cls.options)
    ...
    def tearDownClass(cls) -> None:
        ...
        """
        cls.driver.quit()
        if cls.pipewire_pulse is not None:
            cls.pipewire_pulse.terminate()
    ...
    def test_1_sink_1_list_device(self) -> None:
        ...
        Add a null sink
        """
        self.pipewire_pulse = subprocess.Popen(["pipewire-pulse"], stdout=sys.stderr, stderr=sys.stderr):
    ...
    def test_1_sink_2_bug472245_high_volume_on_startup(self) -> None:
        """
        Tests if the volume is set to the correct value on startup
        @see https://bugs.kde.org/472245
        """
        self.driver.quit()
        self.driver = webdriver.Remote(command_executor='http://127.0.0.1:4723', extensions=[SetValueCommand], options=self.options)
        wait = WebDriverWait(self.driver, 30)
        wait.until(EC.presence_of_element_located((AppiumBy.NAME, "150%")))
