...
class AppletTest(unittest.TestCase):
    ...
    def test_2_microphone_device(self) -> None:
        """
        Add a virtual source
        """
        with subprocess.Popen(["pipewire-pulse"], stdout=sys.stderr, stderr=sys.stderr):
            description: Final = "Virtual_Dummy_Input"
            subprocess.check_call(["pactl", "load-module", "module-virtual-source", "source_name=VirtualMic", f"source_output_properties=device.description={description}"])
            self.driver.find_element(AppiumBy.NAME, description)
        self.driver.find_element(AppiumBy.NAME, "No output or input devices found")
