...
constexpr auto MONITOR_RULES = {MONITOR_ALSA_RULES, MONITOR_BLUEZ_RULES};
...
void writeOverrides(const QHash<QString, QVariantMap> &overrides)
{
    ...
    for (const auto &[name, override] : overrides.asKeyValueRange()) {
        ...
        alsaRules.append(rule);
        acceptedOverrides.append(name);
    }

    QJsonArray bluezRules;
    for (const auto &[name, override] : overrides.asKeyValueRange()) {
        ...
        rule.insert("matches"_L1,
                    QJsonArray({QJsonObject{{"node.name"_L1, name}, //
                                            {"port.monitor"_L1, "!true"_L1}}}));
        rule.insert("actions"_L1, QJsonObject{{"update-props", QJsonObject::fromVariantMap(override)}});
        bluezRules.append(rule);
        acceptedOverrides.append(name);
    }

    if (acceptedOverrides.size() != overrides.size()) {
        qCWarning(PLASMAPA) << "Unmapped devices!" << acceptedOverrides << "versus" << overrides.keys();
    }
...
void DeviceRenameSaver::setModels(QList<DeviceRenameModel *> models)
{
    if (models == m_models) {
        return;
    }
