...
void VolumeMonitor::resetStreamIndex()
{
    m_streamIdx = -1;
...
void VolumeMonitor::classBegin()
{
}

void VolumeMonitor::componentComplete()
{
    createStream();
}
...
void VolumeMonitor::createStream()
{
    ...
    if (!(m_stream = pa_stream_new(Context::instance()->context(), "PlasmaPA-Internal-PeakDetect", &ss, nullptr))) {
        qCWarning(PLASMAPA) << "Failed to create stream";
        return;
    }

    if (m_streamIdx != (uint32_t) -1) {
        pa_stream_set_monitor_stream(m_stream, m_streamIdx);
    }
