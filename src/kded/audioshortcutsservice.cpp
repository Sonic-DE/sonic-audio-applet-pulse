...
void AudioShortcutsService::applyGlobalMute(const QModelIndex &index, const bool globalMute, QStringList &globalMuteDevices)
{
    ...
    if (globalMute) {
        ...
        if (isUnmuted) {
            if (!globalMuteDevices.contains(deviceId)) {
...
void AudioShortcutsService::handleNewSink(const QModelIndex &parent, int first, int last)
{
    ...
    for (int i = first; i <= last; ++i) {
        applyGlobalMute(m_sinkModel->index(i, 0), globalMute, globalMuteDevices);
    }
...
void AudioShortcutsService::handleDataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QList<int> &roles)
{
    ...
    for (int i = topLeft.row(); i <= bottomRight.row(); ++i) {
        ...
        const bool volumeChanged = roles.contains(m_sinkModel->role("Volume"_ba)); // TODO: Is this role name correct?
        ...
        if (activePortIndexChanged) {
            ...
            applyGlobalMute(index, globalMute, globalMuteDevices);
...
void AudioShortcutsService::enableGlobalMute()
{
    ...
    for (int i = 0; i < m_sinkModel->rowCount(); ++i) {
        applyGlobalMute(m_sinkModel->index(i, 0), true, globalMuteDevices);
    }
...
void AudioShortcutsService::disableGlobalMute()
{
    QStringList globalMuteDevices = m_globalConfig->globalMuteDevices();
    for (int i = 0; i < m_sinkModel->rowCount(); ++i) {
        applyGlobalMute(m_sinkModel->index(i, 0), false, globalMuteDevices);
    }
