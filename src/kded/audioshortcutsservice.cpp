...
void AudioShortcutsService::handleDataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QList<int> &roles)
{
    ...
    for (int i = topLeft.row(); i <= bottomRight.row(); ++i) {
        ...
        const auto volumeChanged = roles.indexOf(m_sinkModel->role("Volume"_ba)) != -1; // TODO: Is this role name correct?
        ...
        if (globalMute && (volumeChanged || (mutedChanged && isUnmuted))) {
            // Changes to volume or unmuting break global mute
            qDebug() << "GlobalMute: Breaking global mute" << deviceId << volumeChanged << mutedChanged << isUnmuted;
        ...
        if (globalMute && activePortIndexChanged && isUnmuted) {
