...
void ListItemMenu::setCardModel(QPulseAudio::CardModel *cardModel)
{
    ...
    if (m_cardModel) {
        ...
        connect(m_cardModel, &CardModel::dataChanged, this,
...
bool ListItemMenu::checkHasContent()
{
    ...
    if (device) {
        ...
        if (ports.length() > 1) {
            ...
            // TODO do we need a bounds check or is activePort always valid from PA?
            auto *activePort = qobject_cast<Port *>(ports.at(device->activePortIndex()));
            ...
            for (auto *portObject : ports) {
                auto *port = qobject_cast<Port *>(portObject);
            ...
            for (int i = 0; i < m_cardModel->rowCount(); ++i) {
                ...
                if (card->index() == device->cardIndex()) {
                    ...
                    for (auto *profileObject : profiles) {
                        auto *profile = qobject_cast<Profile *>(profileObject);
...
QMenu *ListItemMenu::createMenu()
{
    ...
    if (auto *device = qobject_cast<QPulseAudio::Device *>(m_pulseObject.data())) {
            ...
            if (m_itemType == Sink) {
                switchStreamsAction = menu->addAction(
                    QIcon::fromTheme(QStringLiteral("audio-on"),
                        QIcon::fromTheme(QStringLiteral("audio-ready"),
                ...
                switchStreamsAction = menu->addAction(
                    QIcon::fromTheme(QStringLiteral("mic-on"),
                        QIcon::fromTheme(QStringLiteral("mic-ready"),
        ...
        // Ports
        const auto ports = device->ports();
        const bool activePortUnavailable = qobject_cast<Port *>(ports.at(device->activePortIndex()))->availability() == Port::Unavailable;

        QMap<int, Port *> availablePorts;
        for (int i = 0; i < ports.count(); ++i) {
            auto *port = qobject_cast<Port *>(ports.at(i));
        ...
        if (availablePorts.count() > 1) {
            ...
            for (auto it = availablePorts.constBegin(), end = availablePorts.constEnd(); it != end; ++it) {
                ...
                if (port->availability() == Port::Unavailable) {
                    if (port->name() == QLatin1String("analog-output-speaker")
                            || port->name() == QLatin1String("analog-input-microphone-internal")) {
        ...
        if (m_cardModel) {
            ...
            for (int i = 0; i < m_cardModel->rowCount(); ++i) {
                ...
                if (candidateCard->index() == device->cardIndex()) {
                    card = candidateCard;
                    break;
                }
            }
            ...
            if (card) {
                ...
                for (int i = 0; i < profiles.count(); ++i) {
                    auto *profile = qobject_cast<Profile *>(profiles.at(i));
                ...
                if (availableProfiles.count() > 1) {
                    ...
                    if (availableProfiles.count() > 5 && !menu->actions().isEmpty()) {
